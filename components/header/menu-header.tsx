...
export const MenuHeader:FC<scrollHeader> = ({handleClickGirl, handleClickContact, handleClickGraphic}) => {
  ...
  const [isActiveRabbit, setIsActiveRabbit] = useState(false)
  ...
  return (
    <nav className={styles.menu}>
      {isActive && isActiveRabbit  && (
        <button className={styles.btnClose} onClick={() => {
          setIsActive(!isActive)
          setIsActiveRabbit(false)
        }}><ExportedImage width={20} height={20} src={close} alt="rabbit" placeholder="blur" /></button>
      )}
      <div className={isActive && isActiveRabbit ? styles.active : styles.menuContainer}>
        <button onClick={ () => {
          setIsActive(!isActive)
          setIsActiveRabbit(false)
          handleClickGirl()
      ...
      <button className={styles.btnRabbit} onClick={() => {
        setIsActiveRabbit(true)
        setIsActive(true)
        }}><ExportedImage width={33} height={34.62} src={scroll > 870 ? rabbitBlack : rabbit} alt="rabbit" placeholder="blur" /></button>
    </nav>
  )
}